...
from src.utils.graphics.old_graphics import auto_contrast_skimage
...
def get_dark_AVG(names, root_path, dark_name="DARK", _zip=True, fit_format=FitsInfo):
    datas, times = get_dark(names, root_path, dark_name, _zip=_zip, fit_format=fit_format)
    data_avg = (np.mean(datas, axis=0))
...
def create_all_png(frequency="5577", _zip=True):
    ...
    for name in names:
        ...
        old_graphics.save_graphics(data, save_path, name[:-8])
...
def get_hist_p(names_files, new_path, start_i=0, end_i=None, _zip=False, counts_checks=4, bins=2000, cut=False,
               percent_to_trim=0.1, fit_format=FitsInfo, dark=False, dark_name="DARK",
    ...
    for i in check_frame:
        ...
        if remove_single_pixels:
            data = old_graphics.remove_single_pixels(data, False, False, False)
            data1 = old_graphics.remove_single_pixels(data1, False, False, False)
        ...
        if Rayleigh:
            ...
            data = old_graphics.calculate_frame_Rayleigh(data, info_f, False)
            data1 = old_graphics.calculate_frame_Rayleigh(data1, info_f1, False)


        if cut:
            data = old_graphics.cut_img(data, percent_to_trim)
            data1 = old_graphics.cut_img(data1, percent_to_trim)
...
def create_image(
        ...
        fit_format=FitsInfo, dark=False, dark_name="DARK", n=10000, _zip=True, remove_single_pixels=False,
    ...
    if correct_matrix is not None:
        corr_matrix = old_graphics.create_correct_matrix(2, 2048, correct_matrix)
    else:
        corr_matrix = None
...
def create_img_for_video(names_files=[], root_path="", first_frame_number=0, last_frame_number=None, flag_info=False, name=None, cut=False,
                         ...
                         fit_format=FitsInfo, dark=False, dark_name="DARK", _zip=True, hists=True, remove_single_pixels=False,
    ...
    for i in range(first_frame_number, last_frame_number):
        ...
        if names:
            info_f = fit_format(info)
            info_f1 = fit_format(info1)
            img = old_graphics.create_img_for_video(data, data1, name=name,
            ...
            img = old_graphics.create_img_for_video(data, data1, name=name, _type=type_diff,
                                                upper_limit=upper_limit, lower_limit=lower_limit, auto_contrast=auto_contrast,
                                                auto_contrast_percentiles=auto_contrast_percentiles)
        # plt.imshow(img)
        # plt.show()
        ...
        if hists:
            diff = data - data1
            img_hist = old_graphics.create_hists(data, data1, diff, diff1, figsize_x=(img.shape[1]+0.5)/100,
...
def create_video(names_files=[], new_path="", start_i=6, end_i=None, name_file="output", flag_info=False, name=None, cut=False,
                 ...
                 name_video_folder="video", dark=False, dark_name="DARK", fit_format=FitsInfo, _zip=True, hists=False, remove_single_pixels=False,
...
def viewing_pictures(names, file_number, new_path, dark=True, n = 120000, _zip=True):
    ...
    mode = old_graphics.print_graphics_cv2_arr(data, data1, data.max(),
    ...
    while True:
        ...
        if dark:
            time = FitsInfo(info).get_datetime()
            data = subtract_noise_frame(dark1, dark2, time1, time2, data, time)
            data = (data - data.min()) / (data.max() - data.min())
            data = (data * n).astype(int)
        ...
        data_cut = old_graphics.cut_img(data, nan=False)
        data1_cut = old_graphics.cut_img(data1, nan=False)

        mode = old_graphics.print_graphics_cv2_arr(data_cut, data1_cut, data_cut.max(),
                                               names=[names[file_number][:-8], names[file_number + 1][:-8]])


def create_heatmap(names=None, new_path=r"", edges=0, start_file=0, end_file=None, log_info=False, title=None, bins=100,
                   auto_contrast=True, cmap="viridis", save_folder=None, fit_format=FitsInfo, _zip=True,
    ...
    for frame in range(start_file+edges, end_file-edges):
        ...
        if auto_contrast:
            data, _, _ = old_graphics.auto_contrast_skimage(data, auto_contrast_percentiles=q)

        result_hist = old_graphics.create_hists(data, data, data,
    ...
    if result_auto_contrast:
        transposed_data, _, _ = old_graphics.auto_contrast_skimage(transposed_data)
...
def create_heatmap1(names=None, new_path=r"", edges=0, start_file=0, end_file=None, log_info=False, title=None, bins=100,
                   auto_contrast=True, cmap="viridis", save_folder=None, limit=None, fit_format=FitsInfo, _zip=True):
    ...
    if auto_contrast:
        ...
        info, data = file.open_gz(name_path, _zip=_zip)
        data_cut = old_graphics.cut_img(data)
        info_for_heatmap[count] = fit_format(info).get_norm_time()

        print(f"max: {np.max(data_cut[~np.isnan(data_cut)])}")

        _, p2, p98 = old_graphics.auto_contrast_skimage(data_cut)
        # p2 -= 2000-500
        # p98 += 1000

        data_c, _, _ = old_graphics.auto_contrast_skimage(data_cut, p2, p98)
        ...
        data_for_heatmap[count] = old_graphics.get_bins_hist(data_c, bins=bins)
        #bins = graphics.get_binss_hist(data_c, bins=bins)
        count += 1
        if log_info:
            print(f"create 0/{count_files}, {p2}, {p98}")
            print(f"max: {data_c.max()}")
            print(old_graphics.get_bins_hist(data_c, bins=bins))
            print(old_graphics.get_binss_hist(data_c, bins=bins))
            print('\n')
            print(bins)
        start_file += 1
...
def save_heat_map(names, new_path, save_folder, type_fits:FitsInfoAbstract=FitsInfo2014, _zip=False,
    ...
    for i in range(start_i, end_i):
        ...
        if use_names:
            ...
            name_file = i
        old_graphics.save_heat_map(diff, save_folder=save_folder, nameFile=name_file, color_bar=False)
...
def write_data_in_file(names, new_path, save_folder, start_i=0, end_i=None,
                       ...
                       save_folder_clusters=None, logs=False, type_fits=FitsInfo2014,
