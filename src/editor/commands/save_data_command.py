...
class SaveDataCommand(BaseCommand):
    ...
    def __init__(self, editor, safe_folder:str, name_file:str=None):
    ...
    def execute(self) -> bool:
        ...
        if not os.path.exists(self.safe_folder):
            os.makedirs(self.safe_folder)

        if self.name_file is None:
            self.name_file = f"{self._editor.target_img.name}_result_matrix"

        result_matrix_safe_folder_data = os.path.join(self.safe_folder, f"{self.name_file}.pkl")
