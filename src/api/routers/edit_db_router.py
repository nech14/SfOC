...
@router.post(f"{API_ROOT}/get_dark_files_db", tags=[ApiTags.EditDatabase])
async def get_dark_files(request: GetDarkFilesDbRequest):
    ...
    async with img_task_semaphore:  # Ограничиваем количество одновременных задач
        ...
        running_tasks_name.append(f"get_dark_files_logic_db {request}")
        running_tasks.append(task)

        task.add_done_callback(lambda t: running_tasks_name.remove(f"get_dark_files_logic_db {request}"))
